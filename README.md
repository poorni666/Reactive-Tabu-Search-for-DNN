...
In non convex search space , the hill climbing algorithm does not work and hence we need something that is better than that. We want to escape the local min in the case you can see in the graph and we have to find the global minimum far away from the local search search space.  Reactive Tabu Search (RTS) improves upon standard Tabu Search by dynamically adjusting the tabu list size and introducing an escape mechanism to avoid repeated solutions and escape local minima.
